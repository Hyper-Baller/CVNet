...
def verify_correspondence(dataset_name, mode=None):
    """ Verify correspondence between train/val/test dataset. 检查t1-t2-label是否一一对应
    ...
    # 修改了这一部分代码
    image_names_temp=np.array(image_names)
    res=0;
    for i in image_names_temp:
        res=res+len(i)
    # print(res)
    # print(len(image_names_temp))
    # len_ans=len(image_names_temp)/3
    image_names = np.unique(np.array(image_names))
    # print(len(image_names))
    if len(image_names) != res/3:
...
def delete_monochrome_image(dataset_name, mode=None):
    ...
    and corresponding image in t1 and t2 directory. 去除掉 全黑的或者全白的图像
    ...
    if mode is None:
        ...
        t2_images_dir = Path(f'./{dataset_name}/{mode}/t2/')
        label_images_dir = Path(f'./{dataset_name}/{mode}/label/')
    #splitext分割了前缀和拓展名 ids每一个对象是Image_1 Image_2等 
    # print(t1_images_dir)
    img_name_sample = listdir(t1_images_dir)[0] #获取该目录下的第一个图片 
    ids = [splitext(file)[0] for file in listdir(t1_images_dir) if not file.startswith('.')] #生成一个列表 
    img_sample = Image.open(str(t1_images_dir) +'/' + str(img_name_sample))
    img_size = img_sample.size[0]  # the image's height and width should be same size只返回高宽

    if not ids:
        raise RuntimeError(f'No input file found in {t1_images_dir}, make sure you put your images there')
    for name in tqdm(ids):
        label_img_dir = list(label_images_dir.glob(str(name) + '.*')) #这个返回的是一个generator对象
...
def crop_img(dataset_name, pre_size, after_size, overlap_size):
    ...
    if ((pre_size - after_size) % (after_size - overlap_size) != 0) or (pre_size % after_size != 0):
        # print(pre_size - after_size % after_size - overlap_size)
...
def crop_whole_image(dataset_name, crop_size):
    ...
    images_path = [Path('WHU/1. The two-period image data/t1/before.tif'),
                   Path('WHU/1. The two-period image data/t2/after.tif'),
                   Path('WHU/1. The two-period image data/label/change_label.tif')
                   ]
    suffix = '.tif'
...
def sample_dataset(dataset_name, mode=None, ratio=None, num=None):
    """ Random sample specified ratio or number of dataset. 随机抽取指定比例的图像并且放到新的文件夹里
...
def convert_tif_to_jpg(tif_file, jpg_file):
    with Image.open(tif_file) as img:
        img.convert('RGB').save(jpg_file, 'JPEG', quality=100)

def convert_bw_tif_to_jpg(tif_file, jpg_file):
    with Image.open(tif_file) as img:
        # 转为灰度图像
        grayscale_img = img.convert('L')
        # 直接保存为 JPEG，质量为 100
        grayscale_img.save(jpg_file, 'JPEG', quality=100)
def covert_tif_to_jpg(dataset_name,mode):
    "将tif文件转化为jpg文件"
    t1_images_dir=Path(f'./{dataset_name}/{mode}/t1/')
    t2_images_dir=Path(f'./{dataset_name}/{mode}/t2/')
    ...
    t1_images_dir_1=f'./{dataset_name}_jpg/{mode}/t1/'
    t2_images_dir_1=f'./{dataset_name}_jpg/{mode}/t2/'
    labels_dir_1=f'./{dataset_name}_jpg/{mode}/label/'

    t1_ids = [splitext(file)[0] for file in listdir(t1_images_dir) if not file.startswith('.')]
    t2_ids = [splitext(file)[0] for file in listdir(t2_images_dir) if not file.startswith('.')]
    t1_name = t1_ids[3]
    t2_name = t2_ids[3]
    t1_img_file = list(t1_images_dir.glob(t1_name + '.*'))
    t2_img_file = list(t2_images_dir.glob(t2_name + '.*'))
    label_file = list(labels_dir.glob(t1_name + '.*'))
    # print(label_file)

    for i in t1_ids:
            t1_img_file = list(t1_images_dir.glob(i + '.*'))
            t2_img_file = list(t2_images_dir.glob(i + '.*'))
            label_file = list(labels_dir.glob(i + '.*'))

            t1_img_file_1 = f'{t1_images_dir_1}/{i}.jpg'
            t2_img_file_1 = f'{t2_images_dir_1}/{i}.jpg'
            label_file_1 = f'{labels_dir_1}/{i}.jpg'
            

            convert_tif_to_jpg(t1_img_file[0],t1_img_file_1)
            convert_tif_to_jpg(t2_img_file[0],t2_img_file_1)
            convert_bw_tif_to_jpg(label_file[0],label_file_1)

if __name__ == '__main__':
    crop_img('DSFN',512,256,0)
    # verify_correspondence('WHU_jpg','train')
    # verify_correspondence('WHU_jpg','test')
    # verify_correspondence('WHU_jpg','val')
    # crop_whole_image("WHU",256)
    # covert_tif_to_jpg("WHU","test")
    # covert_tif_to_jpg("WHU","train")
    # covert_tif_to_jpg("WHU","val")