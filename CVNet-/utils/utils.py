...
def train_val(
        mode, dataset_name,
        dataloader, device, net, optimizer, total_step,
        lr, criterion, metric_collection, to_pilimg, epoch,
        warmup_lr=None, grad_scaler=None,
        best_metrics=None, checkpoint_path=None,
        best_f1score_model_path=None, best_loss_model_path=None, non_improved_epoch=None,lr_scheduler=None,
):
    """
    ...
    for i, (batch_img1, batch_img2, labels, name) in enumerate(tbar):
        ...
        if mode == 'train':
            # using amp
            with torch.cuda.amp.autocast(): #让混合精度没了 
